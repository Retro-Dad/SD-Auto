...
# This script is mirrored from the SteamOS systemd file to mount or unmount a USB drive.
...
# Shared between this and the auto-mount script to ensure we're not double-triggering nor automounting while formatting or vice-versa.
...
if ! flock -n 9; then
    ...
    # Do not return a success exit code; it could put the service in 'started' state without actually mounting
    exit 1
fi
...
send_steam_url()
{
  ...
  if pgrep -x "steam" > /dev/null; then
      # TODO use -ifrunning and check return value - if there was a steam process and it returns -1, the message wasn't sent; need to retry until either steam process is gone or -ifrunning returns 0, or timeout
...
do_mount()
{
    ...
    # Expected reply is of the format {"type":"s","data":["/run/media/deck/home"]}
    ...
    # Create a symlink from /run/media to keep compatibility with apps that use the older mount point (for SD cards only).
    case "${DEVBASE}" in
        mmcblk0p*)
...
do_retrigger()
{
    ...
    # In retrigger mode, we want to wait for steam as the common pattern is starting in parallel with a retrigger
    wait_steam 10
    # TODO any remaining open tasks
    sleep 6
    send_steam_url "addlibraryfolder" "${mount_point}"
}
